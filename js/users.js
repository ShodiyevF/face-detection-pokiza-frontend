...
async function getUsers(){
    ...
    for (const i of data) {
        ...
        role_badge.classList.add('badge', 'badge-pill', i.email ? 'bg-success-light' : 'bg-danger-light')
        employerEdit.classList.add('btn', 'btn-sm', 'btn-white', 'text-success', 'me-2', 'user_update_btn', 'edit_btn')
        employerDelete.classList.add('btn', 'btn-sm', 'btn-white', 'text-danger', 'me-2', 'user_delete_btn', 'delete_btn')
...
async function setUser(){
    userSaveBtn.onclick = async (e) => {
        ...
        const resa = /^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/g.test(userEmailInput.value)
...
async function updateUser(){
    ...
    for (const i of user) {
        i.onclick = async (e) => {
            ...
            const user = (await users()).filter(el => el.id == e.target.dataset.id)[0]
